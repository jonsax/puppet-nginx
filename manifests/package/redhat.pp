  ...
  if $manage_repo {
    case $package_source {
      ...
      default: {
        ...
        # To compile nginx from source
        include gcc
        
        # Update openssl on CentosOS for SPDY support
        class { "openssl":
          src_packages => !$manage_repo
        }
    
        # recompile nginx with SPDY
        # --with-file-aio disabled due to no compiled in support in linux centos 6.4 kernel
        # -mtune=generic -m64 removed due to a compile error

        # make install required Defaults    !requiretty in /etc/sudoers
        build::netinstall { 'nginx':
					  ...
					  postextract_command => template('nginx/build/build-centos.erb')
			  }
