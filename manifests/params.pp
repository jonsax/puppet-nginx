...
class nginx::params {
  ...
  $nx_fastcgi_buffers     = '8 16k'
  $nx_fastcgi_buffer_size = '32k'
  $nx_gzip_types          = 'text/plain text/css application/x-javascript text/xml application/xml application/xml+rss text/javascript'
  $nx_forward_ip          = "off"
  $nx_set_real_ip         = '127.0.0.1'
  $nx_real_ip_header      = 'X-Real-IP'  

  # package name depends on distribution, e.g. for Debian nginx-full | nginx-light
  $nginx_source_version  = '1.8.1'  
  $fastcgi_pass = $operatingsystem ? { centos => 'unix:/var/run/php-fpm/php-fpm.sock', ubuntu => 'unix:/var/run/php5-fpm.sock', default => 'unix:/var/run/php-fpm/php-fpm.sock' }
}
