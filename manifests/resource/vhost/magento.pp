# define: nginx::resource::vhost::magento
#
...
define nginx::resource::vhost::magento (
  ...
  Optional[Hash] $add_header                                                     = undef, # { 'X-Frame-Options' => 'SAMEORIGIN' }, # this is already set by magento
  Boolean $ssl                                                                   = false,
  ...
  Optional[Variant[String, Array]] $access_log                                   = 'off',
  ...
  $mage_root                    = undef,
  $laravel_root                 = undef,
  $mage_mode                    = 'default', ## Magento 2 setting - #default or production or developer
  $api_fastcgi                  = $operatingsystem ? {  centos => 'unix:/var/run/php-fpm/php-fpm.sock',
    ubuntu => 'unix:/var/run/php/php7.0-fpm.sock', default => 'unix:/var/run/php5-fpm.sock' },
  $fallback_fastcgi             = undef,
  $admin_fastcgi                = undef,
  $admin_url                    = 'admin_dncb7h',
  $sitedown_url                 = "https://www.vax.co.uk",
  $ssl_redirect_header          = undef
) {
  ...
  # Check Magento Run Code and Run Type are Defined
  if ($mage_run_code == undef) or ($mage_run_type == undef)  {
    warning('nginx: Please enter a magento run code and type')
  }
  ...
  if $listen_port != $ssl_port {
    ...
    if ($custom_template == undef) {
      concat::fragment { "${name_sanitized}-content":
        target  => $config_file,
        content => template('nginx/vhost/vhost_magento.erb'),
        order   => '002',
      }
    } else {
      concat::fragment { "${name_sanitized}-content":
  ...
  if $ssl {
    ...
    if ($custom_template == undef) {
      concat::fragment { "${name_sanitized}-ssl-content":
        target  => $config_file,
        content => template('nginx/vhost/vhost_magento.erb'),
        order   => '701',
      }
